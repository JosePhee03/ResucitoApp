...
class Chord {
    ...
    companion object {
        ...
        fun chordTypeFromString(chordTypeStr: String): ChordType? {
            val match = chordRegex.matchEntire(chordTypeStr) ?: return null
            val noteMatch = match.groupValues[1]
            return try {
                ChordType.valueOf(chordTypeStr)
            } catch (ex: IllegalArgumentException) {
                null
            }
        }
